...
class statsV2 {
    ...
    function getStat(){
        ...
        //utils::vd($elapsed_time . ' = '. $this->task_count);
        return round($tasks_per_sec, 1);
    }
	...
	private $reset = false;
    function hasPassed($interval = 10){
    ...
    private function reset(){
		$this->reset = false;
        $this->store->setValue($this->process_name, json_encode(array(
            'Tasks Per Second' => $this->getTasksPerSecond(),
            'Tasks Completed' => $this->getTasksCompleted(),
            'Progress' => $this->getPercentage(),
            'Peak RAM Usage' => $this->getPeakRam(),
            'PID' => $this->pid,
        )));
    ...
    function getSessionTime(){
        return round($this->timer->getSessionTime(), 2);
    }

    function getTasksPerSecond(){
        return $this->getStat();
    }

    function getPeakRam(){
        return $this->benchmark->getRamUsage();
    }
    ...
    function getPercentage(){
        $percent = 'N/A';
        if($this->getTaskCount() !== 0 && $this->total_task_count !== false){
            $percent = round(100*($this->getTaskCount()/$this->total_task_count), 2);
        }
