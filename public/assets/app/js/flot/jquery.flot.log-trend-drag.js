...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        function setData(d) {
            series = parseData(d);
                        if (options.trendLines.show){
                                var trendlines = [];
                                var slen = series.length;
                                for (var i=0; i<slen; i++){
                                        var s = series[i];
                                        s.trendLineIndex = null;
                                        if (s["trendLines"] == null || s["trendLines"]["show"] != false){
                                                // add a copy of the line as a placeholder for the trendline
                                                var temp = {};
                                                temp.data = $.extend(true, [], s.data);
                                                temp.trendLines = $.extend(true, {}, s.trendLines);
                                                temp.trendPoints = $.extend(true, {}, s.trendPoints);
                                                temp.isTrendline = true;
                                                series.push(temp);
                                                s.trendLineIndex = series.length-1;
                                        }
                                }
                        }
        ...
        function parseOptions(o) {
            if (o.xaxis != null && o.xaxis.base > 0 && !(o.xaxis.ticks instanceof Array))
                o.xaxis.tickFormatter = function(n, axis) {
                    return Math.pow(o.xaxis.base,n).toFixed(axis.tickDecimals);
                }

            if (o.yaxis != null && o.yaxis.base > 0 && !(o.yaxis.ticks instanceof Array)){
                o.yaxis.tickFormatter = function(n, axis) {
                    return Math.pow(o.yaxis.base,n).toFixed(axis.tickDecimals);
                }
                        }
            if (o.x2axis != null && o.x2axis.base > 0 && !(o.x2axis.ticks instanceof Array))
                o.x2axis.tickFormatter = function(n, axis) {
                    return Math.pow(o.x2axis.base,n).toFixed(axis.tickDecimals);
                }

            if (o.y2axis != null && o.y2axis.base > 0 && !(o.y2axis.ticks instanceof Array))
                o.y2axis.tickFormatter = function(n, axis) {
                    return Math.pow(o.y2axis.base,n).toFixed(axis.tickDecimals);
                }
        ...
        function fillInSeriesOptions() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                // copy the rest
                                // if a trendline, copy from trendline options.
                                // trendlines will not show up as bars (they're trend *lines*, right?)
                                // turn off interactive features on trendlines.
                                if (s['isTrendline'] == true){
                        s.lines = $.extend(true, {}, options.trendLines, s.trendLines);
                        s.points = $.extend(true, {}, options.trendPoints, s.trendPoints);
                        s.bars = $.extend(true, {}, options.bars);
                                        s.label = s.lines.label;
                                        s.bars.show = false;
                                        s.clickable = false;
                                        s.hoverable = false;
                                        s.dragable = false;
        ...
        function processData() {
            ...
            for (var i = 0; i < series.length; ++i) {
                ...
                for (var j = 0; j < data.length; ++j) {
                    if (data[j] == null) // invalid point
                        continue;
                    var x = data[j][0], y = data[j][1];
                                       
                                        var yaxisBase = options.yaxis.base;
                                        var xaxisBase = options.xaxis.base;
                                        if (axisy == axes.y2axis)
                                                yaxisBase = options.y2axis.base;
                                        if (axisx == axes.x2axis)
                                                xaxisBase = options.x2axis.base;

                    if ( yaxisBase > 1 ) {
                                                if (y > 0) {
                            y = Math.log(y) / Math.log(options.yaxis.base);
                                                }
                                                else y = null;
                    }
                    ...
                    if ( xaxisBase > 1 ) {
                                                if (x > 0) {
                            x = Math.log(x) / Math.log(options.xaxis.base);
                                                }
                                                else x = null;
                    }

                    data[j][0] = x, data[j][1] = y;
                    ...
                    if (x == null || y == null || isNaN(x) || isNaN(y))
                        data[j] = null; // mark this point as invalid
                                ...
                                if (series[i]['isTrendline'] == true) {
                                        series[i].data = linearFit(data);
                                }
            }
        ...
        function constructCanvas() {
            ...
            if (options.selection.mode != null || options.crosshair.mode != null
                || options.grid.hoverable || options.dragable) {
                // FIXME: temp. work-around until jQuery bug 1871 is fixed
                eventHolder.each(function () {
                    this.onmousemove = onMouseMove;
                });

                if (options.selection.mode != null || options.dragable)
                    eventHolder.mousedown(onMouseDown);
            }

            if (options.crosshair.mode != null || options.dragable)
        ...
        function onMouseMove(ev) {
            ...
            if (selection.active) {
                ...
                updateSelection(lastMousePos);
                        ...
                        else if (drag.active) { // assume selection mode and dragging incompatible for now
                                updateDrag(lastMousePos);
                        }
        }
        ...
        function onMouseDown(e) {
            ...
            if (document.ondrag !== undefined && workarounds.ondrag == null) {
                ...
                $(document).one("mouseup", onSelectionMouseUp);
                    ...
                    drag.item = findNearbyItem(canvasX, canvasY, function (s) { return s["dragable"] != false; });
        ...
        function onClick(e) {
                        // stop dragging when mouse is let up.
                        drag.active = false;
                        drag.item = null;
            if (clickIsMouseUp) {
                clickIsMouseUp = false;
        ...
        function triggerClickHoverEvent(eventname, event, seriesFilter) {
            ...
            if (item) {
                ...
                item.pageY = parseInt(item.series.yaxis.p2c(item.datapoint[1]) + offset.top + plotOffset.top);
                                retitem = { dataIndex: item.dataIndex, datapoint: [item.datapoint[0], item.datapoint[1]], pageX: item.pageX, pageY: item.pageY, series: item.series, seriesIndex: item.seriesIndex };
                                ...
                                if ( yaxisBase > 1 ) {
                                        retitem.datapoint[1] = Math.exp(p[1]*Math.LN10);
                    }

                                if ( xaxisBase > 1 ) {
                                        retitem.datapoint[0] = Math.exp(p[0]*Math.LN10);
                    }
            }
            ...
            target.trigger(eventname, [ pos, retitem ]);
        }
        ...
        function updateSelection(pos) {
            ...
            if (selectionIsSane()) {
                ...
                function updateDrag(pos) {
                        ...
                        if (series[sidx]['trendLineIndex'] != null) {
                                series[series[sidx].trendLineIndex].data = linearFit(series[sidx].data);
                        }
                       
                        draw();
                       
                        var s = series[drag.item.seriesIndex];
                        var rety = newy;
                        var retx = newx;
                        ...
                        target.trigger('plotSeriesChange', [drag.item.seriesIndex, drag.item.dataIndex, retx, rety])
                }
    ...
    function parseColor(str) {
        ...
        function linearFit(data) {
                ...
                slopit = regression(x, y,'linear');
               
                for (i=0; i<x.length; i++){
                    res = slopit[0]*x[i] + slopit[1];
                    ypred.push([x[i], res]);
                }
